...
* Plugin URI: https://sangams.com.np/toolbox/flash-footer-editor/
...
if (! function_exists('flash_custom_credits')) {
	...
	function flash_custom_credits(){
		$credit_value ='?>'. '<div class="copyright"><span class="copyright-text">Copyright &copy; ' . date('Y') . ' <a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" ><span>' . get_bloginfo( 'name', 'display' ) . '.</span></a> ' .get_option('flash_custom_credits'). '</span></div>'.'<?php';
		echo $credit_value;
	}
